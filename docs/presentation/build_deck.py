...
c.setFillColor(WHITE)
c.setFont("Sarabun-Semi", 20)
c.drawString(64, 140, "Phirawit Jitnarong (bravforcode) — ทีมคนเดียว (Solo)")
c.drawString(64, 112, "github.com/bravforcode/Solven")
c.drawString(64, 84, "solven.vercel.app — ทดลองใช้งานจริงได้ (live demo)")
c.setFillColor(AMBER)
...
kicker("Root Cause · สพฐ. + TDRI", y=598)
cards = [
...
c.setFillColor(MUTED)
c.setFont("Sarabun", 16)
p(MUTED_P, "LLM: ระหว่างพัฒนาใช้ API (Anthropic/OpenAI) · สเกลจริงใช้ open-source Thai model บน AIS Cloud — ต้นทุนต่ำ + ข้อมูลอยู่ในไทย (data sovereignty)", 720, 120, 500)
footer(6)
...
for i, (t, d, col) in enumerate(ais):
    ...
    c.setFillColor(WHITE)
    c.setFont("Sarabun-Bold", 26)
    c.drawString(x + 26, 486, t)
    c.setFont("Sarabun", 18)
    p(WHITE_SM, d, x + 26, 350, cw - 52, 110)
c.setFillColor(AMBER)
...
c.setFillColor(GREEN_DARK)
c.setFont("Sarabun-Bold", 24)
c.drawString(88, 548, "ทีม Solven — สมัครคนเดียว (Solo)")
c.setFillColor(INK)
c.setFont("Sarabun", 19)
p(BODY, "• Phirawit Jitnarong (bravforcode) — AI/data systems, full-stack, multi-agent orchestration (LangGraph)\n• Prior art: enterprise-agent-os, adminmate-ai (production multi-agent systems)\n• ลงพื้นที่คุยกับครูจริง 2 โรงเรียนในจังหวัดแพร่ ก่อนออกแบบ solution", 88, 396, 520)
c.setFillColor(WHITE)
c.setStrokeColor(GREEN)
c.setLineWidth(2)
...
c.drawString(64, 220, "solven.vercel.app — live demo ทดลองใช้งานได้จริง")
c.setFillColor(MUTED)
c.setFont("Sarabun", 17)
c.drawString(64, 180, "ขอขอบคุณ AIS · กระทรวงศึกษาธิการ · สพฐ. · NIA — ครูไทยควรได้เวลาคืน")
footer(10)
